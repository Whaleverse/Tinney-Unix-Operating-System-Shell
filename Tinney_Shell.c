...
void add_info(int job_N, int status, pid_t w);
...
void eval(char *cmdline) 
{
  ...
  int bg, shall_act, pell;                /* should the job run in bg or fg? */
  shall_act = 0;
  pell = 1;
  //int bg2 = 0;
  ...
  //       If cmd2 is NULL, then there is only one command
	...
	int excude = execv(argv1[0], argv1);
	bool isdoneit;
	if (excude < 0){
	  isdoneit = true;
	}
	...
	while(isdoneit){
	  printf("%s: The command is not dound\n", argv1[0]);
	  isdoneit = false;
	  exit(0);
	}
	...
	if ((pid = fork()) == 0){
	  ...
	  if (tube > 0){
	    ...
	    printf("The command is the error.");
	  }
	
	  int excudeN = execv(argv2[0],argv2);
	  bool isdone;
	  if (excudeN < 0){
	    isdone = true;
	  }
    ...
    bool builted = builtin_cmd(argv1);
    if(!builted) 
      {   
	int j;
	for (j = 0; j < shall_act; j++) {
	  int myjob = pid;
	  if (myjob < -1){
	    printf("fall job");
	  }
...
void do_bg(int jid) 
{
  ...
  int job_number = putinfo(jid);
  if(myjob != NULL){
    if (job_number > -1){
      if(myjob->state == ST){
	kill((myjob->pid)*-1, SIGCONT);
      }
    }
    else{
      job_number ++;
